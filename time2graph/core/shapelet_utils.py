    ...
    def __main__(pid, args, queue):
        ...
        for time_series_segments in args:
            ...
            for cnt in range(n_candiates):
                ...
                # Check if dims is greater than zero before reshaping
                if dims > 0:
                    ...
                    print(f"Skipping reshape due to empty segment for candidate {cnt}")
                    continue
...
def __shapelet_distance_factory(
    ...
    def __main__(pid, args, queue):
        ...
        for time_series in args:
            ...
            tmp = np.zeros((num_segment, len(shapelets)), dtype=np.float64)
...
def shapelet_distance(
    ...
    sdist = np.array(parmap.run(data=list(time_series_set)), dtype=np.float64).reshape(
...
def transition_matrix(
    ...
    if multi_graph:
        ...
        gcnt = 1
    tmat = np.zeros((gcnt, num_shapelet, num_shapelet), dtype=np.float64)
...
def adjacent_matrix(
    ...
    tmat = np.zeros((num_time_series, num_shapelet, num_shapelet), dtype=np.float64)
    for tidx in range(num_time_series):
        for sidx in range(num_segment - 1):
            src_dist = sdist[tidx, sidx, :]
            dst_dist = sdist[tidx, sidx + 1, :]
...
def __embedding2mat(fpath, num_vertices, embed_size):
    mat = np.zeros((num_vertices, embed_size), dtype=np.float64)
...
def graph_embedding(tmat, num_shapelet, embed_size, cache_dir, **deepwalk_paras):
    ...
    return np.array(ret, dtype=np.float64).reshape(
