...
def __shapelet_candidate_loss(
    ...
    **kwargs
    ...
    max_iters, optimizer = 1, kwargs.get("optimizer", "Adam")
    if optimizer == "Adam":
    ...
    while cnt < max_iters:
        ...
        for x, y in dataloader:
            ...
            mean = torch.mean(dist_tensor)                
            std =torch.std(dist_tensor).clamp(min=1e-6)
            ...
            for k in range(1, len(lb_idx)):
                ...
                mean_src = torch.mean(src)
                std_src = torch.std(src).clamp(min=1e-6)
                mean_dst = torch.mean(dst)
                std_dst = torch.std(dst).clamp(min=1e-6)
                
                #look for nan values in the mean and std and replace them by the possible minimum value different from 0
                if torch.isnan(mean_src):
                    mean_src = 1e-6
                if torch.isnan(std_src):
                    std_src = 1e-6
                if torch.isnan(mean_dst):
                    mean_dst = 1e-6
                if torch.isnan(std_dst):
                    std_dst = 1e-6                    
                                                    
                loss -= torch.abs(
                    torch.distributions.kl.kl_divergence(
                        Normal(mean_src, std_src),
                        Normal(mean_dst, std_dst)
                    )
                )
                main_loss -= torch.abs(
                    torch.distributions.kl.kl_divergence(
            ...
            if gpu_enable:
                current_loss = float(loss.cpu().data.numpy().item())
                current_main_loss = float(main_loss.cpu().data)
                current_penalty_loss = float(penalty_loss.cpu().data.item())
            else:
                current_loss = float(loss.data.numpy().item())
                current_main_loss = float(main_loss.data)
                current_penalty_loss = float(penalty_loss.data)
            loss_queue.enqueue(current_loss)
            
            #se o modelo está com os gradientes nulos, reinicia os fatores locais e globais
            if (
...
def __shapelet_candidate_loss_factory(
    ...
    **kwargs
    ...
    def __main__(pid, args, queue):
        ret = []
        for cand in args:
            local_factor, global_factor, loss, main_loss, penalty = (
