...
def func_wrapper(args):
    dill_work_func, slc, report_queue = args
    
    # Unpickle the function
    work_func = dill.loads(dill_work_func)
    
    # Run the work function, which is assumed to return a list of results
    res = work_func(mp.current_process().ident, slc, report_queue)
    
    # If the work function uses a Normal distribution, ensure std is clamped
    for result in res:
        if isinstance(result, Normal):
            result.scale = result.scale.clamp(min=1e-6)
